...
.tbl-scroll-body table td{
    padding:10px 11px;font-size:13px;color:#1a1a1a;border-bottom:1px solid #ebebeb;
    overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
}
.tbl-scroll-head table th{
    font-size:11px;font-weight:700;color:#999;text-transform:uppercase;
    letter-spacing:.5px;padding:12px 11px;border-bottom:none;
}
...
<div class="dash-wrap">
    ...
    <div class="dash-header">
        ...
        {{-- Reemplaza el botón Generar Tarjetas por este formulario --}}
<form method="POST" action="{{ route('tarjetas.generar') }}" style="display:inline">
    @csrf
    <button type="submit" class="btn-accion btn-generar">
        🪪 Generar Tarjetas
    </button>
</form>
            ...
            @if(session('success'))
                <div style="margin-top:1rem;padding:12px 16px;background:#EAF3DE;border-radius:8px;color:#3B6D11;font-size:13px;">
                    {{ session('success') }}
                </div>
            @endif

            @if(session('errores') && count(session('errores')) > 0)
                <div style="margin-top:.5rem;padding:12px 16px;background:#FCEBEB;border-radius:8px;color:#A32D2D;font-size:12px;">
            ...
            <button class="btn-accion btn-enviar" onclick="return false;">
                📨 Enviar Tarjetas
            </button>
        </div>
    </div>
    ...
    <div class="tbl-wrap">
        ...
        @if($asistentes->count() > 0)
            <div class="tbl-footer">
                Mostrando {{ $asistentes->count() }} registro(s)
            </div>
        @endif
    </div>
